...
>
> 当前版本：v1.5（2026-06-25）；本文件是后续计划、进度、复盘与调整的唯一事实来源。
...
每日表格中的 `Gxx` 指 AgentGuide 仓库文档，`Rxx` 指外部学习资料（官方优先，中文教程作辅助），`Txx` 指已克隆到本地的教程仓库。资源列会尽量标明当天应阅读的章节。
...
| R30 | [菜鸟教程：Python3 教程](https://www.runoob.com/python3/python3-tutorial.html) |

## 3.3 本地教程仓库

| 编号 | 仓库与本地位置 | 在本计划中的定位 |
|:---:|:---|:---|
| T01 | [Learn Claude Code](https://github.com/shareAI-lab/learn-claude-code)；本地 `D:\Study\AiAgent\Code\learn-claude-code` | 主要代码实验课。当前推荐主线为 `s01`～`s20`；按机制选学 Agent Loop、工具、权限、Hooks、上下文压缩、Memory、Skill、MCP、错误恢复和 Multi-Agent，不与旧版 12 章编号混读。 |
| T02 | [claw0](https://github.com/shareAI-lab/claw0)；本地 `D:\Study\AiAgent\Code\claw0` | 后期生产架构案例。重点选学 Session、Memory、Channel、投递队列、容错和并发；`s01`～`s02` 与 T01 重复，默认跳过。 |
| T03 | [hello-agents（Datawhale）](https://github.com/datawhalechina/hello-agents)；本地 `D:\Study\AiAgent\Code\hello-agents` | 中文理论教材与项目对照。选读 Agent 范式、RAG、Context、LangGraph、MCP、Evaluation、DeepResearch、Web Agent 和 Agentic RL；不要求逐章通读。 |

当前版本将三套教程分散到 **30 个既有学习日** 中，以替换部分原有阅读或提供代码对照，不增加 238 小时总预算。

### 教程仓库使用规则

- **计划为主，教程为辅**：每日目标与验收以本文件为准；教程只替换当天部分阅读或实验，不额外增加学习时长。
- **一天一个仓库、一个机制、一个产出**：同一天出现两个编号时，只把第二份资料用于快速对照，不展开通读。
- **教程日 60 分钟模板**：`5 分钟回忆 + 15 分钟读指定 README/小节 + 15 分钟运行示例 + 20 分钟追踪或在自己的项目中重写关键机制 + 5 分钟记录机制卡片`。
- **克隆仓库默认只读**：实验优先在临时目录运行；真正要保留的代码在自己的练习项目或 Paper Agent 中重新实现，避免污染教程仓库，也避免大段复制。
- **官方资料优先**：涉及 API、协议、安全和框架当前行为时，以 `Rxx` 官方文档为准；`Txx` 用来帮助理解机制和比较架构。
- **控制阅读范围**：大章节只阅读当日任务相关小节和 3～5 个核心函数。60 分钟未完成时标记 `🔁`，不把整章顺延成新的必修课。
- **公开发布前核对许可**：引用教程观点时注明来源；复用代码前重新查看对应仓库及子项目的许可证。
- **允许灵活调整**：周复盘时可以替换章节、降低为选读或删除已重复内容，但不能删除当周核心实作与验收。

# 阶段 A：工程基础与 Agent 最小闭环

## 第 1 周：路线、环境与 Python 入门

**本周验收**：环境可用；理解学习边界；完成 Python 基础复健；能运行带输入校验的 Python 小程序。

| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ✅ | Day 1 / 06-22 | 浏览路线；写目标岗位、每日 1 小时和算法独立安排 | G01；G02；G29「适合谁」 | [day01-goal.md](../06-notes/day01-goal.md) 写清目标与边界 |
| ✅ | Day 2 / 06-23 | 检查 Python、Git、编辑器和虚拟环境 | R01「Using Python」；R02「起步」 | 保存版本与环境记录 |
| ✅ | Day 3 / 06-24 | 复健变量、字符串、数字、输入输出和 f-string | R01「3. Python 速览」；R30「基本数据类型」「数字」「字符串」「输入和输出」 | 写可运行的单位换算器 |
| ✅ | Day 4 / 06-25 | 复健 `if/for/while`，顺手看列表/字典推导式 | R01「4. 控制流工具」；R30「条件控制」「循环语句」「推导式」 | 换算器有菜单和退出逻辑，并至少用 1 个推导式 |
| ✅ | Day 5 / 06-26 | 复健函数、参数、返回值，补 lambda 和作用域 | R01「4.8 Defining Functions」；R30「函数」「Lambda」「命名空间和作用域」 | 拆出至少 3 个函数，并写 1 个小型高阶函数示例 |
| ✅ | Day 6 / 06-27 | 复健 list、dict、set，补切片、解包和常用遍历模式 | R01「5. 数据结构」；R30「列表」「元组」「字典」「集合」 | 写联系人内存模型，包含增删查和格式化输出 |
| ✅ | Day 7 / 06-28 | 周复盘；不看旧代码重写单位换算器或联系人模型 | G03「Day 7」；R30 按卡点回看 | 写完成率、卡点、下周动作 |

### Week 1 周复盘（2026-06-28）

- Agent 学习完成：7 / 7 天
- Agent 学习投入：7 / 7 小时
- 本周最重要产出：
  - 明确 34 周学习边界：Agent 学习与 LeetCode 式算法训练分开统计。
  - 完成 Python 基础复健：变量、字符串、输入输出、控制流、推导式、函数、lambda、作用域。
  - 完成两个小型验收程序：单位换算器、学习任务统计器。
  - 完成 Day 6 联系人内存模型：使用 list/dict/set，覆盖增删查、切片、解包、遍历和格式化输出。
- 已掌握：
  - 能把简单脚本拆成多个函数，并用字典保存配置或 dispatch 信息。
  - 能用列表/字典/集合组织小规模内存数据。
  - 能用推导式、`enumerate`、`items()`、切片和解包完成常见遍历与转换。
- 仍然模糊：
  - 文件读写、JSON 持久化、异常边界和模块拆分还未进入系统练习。
  - `dict[str, object]` 这种临时模型可用，但后续需要升级为 dataclass / 类型更明确的数据模型。
- 最大阻塞：
  - 06-27 未按当天节奏打卡，Day 6 已补齐；下一周重点是恢复每天固定 1 小时节奏。
- 一个失败案例及原因：
  - 失败案例：Day 6 没有在 06-27 当天完成打卡。
  - 原因：学习节奏中断后没有立即用 60 分钟闭环补齐。
  - 下次改法：当天只保留一个核心验收；无法完成时标记 `🔁`，周复盘日补缺，不额外扩展资料。
- 下周保留：
  - 每天一个可运行脚本或一个明确工程改动。
  - 继续围绕联系人模型演进，避免换题导致上下文分散。
- 下周删减：
  - 不额外扩展 Python 资料；优先做文件、异常、模块、venv、dataclass 和测试前置能力。
- 是否需要调整未来日期：
  - 暂不调整。06-29 按 Day 8 进入第 2 周。

下周动作：

- Day 8：给联系人模型增加 JSON 保存/读取，文件操作全部使用 `with`。
- Day 9：处理坏 JSON、自定义异常和可读错误。
- Day 10：把联系人脚本拆成 2～3 个模块，并保留命令行入口。

## 第 2 周：文件、异常与 Python 工程

**本周验收**：联系人 CLI 可持久化；目录和依赖结构清楚；补齐 OOP、dataclass、类型注解、迭代器/生成器和上下文管理器等 Agent 工程常用 Python 特性。

| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ✅ | Day 8 / 06-29 | 学文件读写、JSON 和 `with` 上下文管理 | R01「7. 输入和输出」；R30「输入和输出」「File 方法」 | 联系人可保存到 JSON，文件操作全部使用 `with` |
| ✅ | Day 9 / 06-30 | 学异常处理，补自定义异常和失败边界 | R01「8. 错误和异常」；R30「错误和异常」 | 坏 JSON 返回可读错误，并有 1 个自定义异常 |
| ✅ | Day 10 / 07-01 | 学模块、包和命令行入口 | R01「6. 模块」；R30「模块」「命令行参数」 | 将脚本拆成 2～3 个模块，并能从命令行运行 |
| ✅ | Day 11 / 07-02 | 学虚拟环境和依赖文件 | R01「12. 虚拟环境和包」 | 建立 venv 和依赖说明 |
| ✅ | Day 12 / 07-03 | 学类、dataclass、类型注解和迭代器/生成器直觉 | R01「9. 类」；R30「面向对象」「迭代器与生成器」 | 定义 `Contact` 数据模型，并写 1 个遍历联系人结果的生成器 |
...
| ✅ | Day 31 / 07-22 | 接入一个真实 provider，密钥用本地忽略配置 | R07 | 真实调用可运行且仓库无密钥 |
...
| ⬜ | Day 41 / 08-01 | 对比 ReAct、Plan-and-Execute 与 Reflection | G07；T03 第 4 章 Plan-and-Solve、Reflection 小节 | 写选择表 |
...
| ⬜ | Day 45 / 08-05 | 实现 Tool Registry 和 dispatch 表 | G15「L3：工具层」；T01 `s02_tool_use/code.py` 的 registry/dispatch | 新增工具不改核心循环 |
| ⬜ | Day 46 / 08-06 | 解析结构化 tool call 并执行 | R08 | 单次工具调用跑通 |
| ⬜ | Day 47 / 08-07 | 实现最多 5 步的 Agent Loop | G06「最小实现」；T01 `s01_agent_loop/README.md` 与 `code.py` | 多步任务可结束 |
...
| ⬜ | Day 51 / 08-11 | 每步写 trace，并生成 run id | G15「Observability」；T01 `s04_hooks/README.md` 的 hook 思路 | 一次运行可完整回放 |
...
| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ⬜ | Day 57 / 08-17 | 理解 RAG 全链路 | G10「概述」；T03 第 8 章 RAG 相关小节 | 画 RAG 流程图 |
...
| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ⬜ | Day 113 / 10-12 | 对照项目蓝图和成熟案例列 v1 缺口 | G20「实现里程碑」；T03 第 14 章 DeepResearch 架构相关小节 | P0/P1 缺口表；只比较，不照搬 |
...
| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ⬜ | Day 120 / 10-19 | 区分 Prompt 与 Context Engineering | G11「核心概念」；T03 第 9 章开篇与核心概念 | 写对比表 |
| ⬜ | Day 121 / 10-20 | 将 context 分为 task/evidence/history/trace | G11「组成部分」 | 画输入顺序 |
| ⬜ | Day 122 / 10-21 | 实现 context builder | G11「长上下文问题」 | 输出结构稳定 |
| ⬜ | Day 123 / 10-22 | 实现工具结果截断和引用保留 | G15「L3」；T01 `s08_context_compact/README.md` 的分层压缩 | 长结果不过载 |
...
| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ⬜ | Day 127 / 10-26 | 理解 working/episodic/semantic memory | G12「三层记忆架构」；T01 `s09_memory/README.md` | 给项目数据分类 |
| ⬜ | Day 128 / 10-27 | 设计什么值得写入 | G12「什么时候存储」 | 写入规则可测试 |
| ⬜ | Day 129 / 10-28 | 用 JSONL/SQLite 保存 episodic memory | G12「自己实现简单 Memory」；T02 `sessions/zh/s03_sessions.md` | 可写入读取 |
| ⬜ | Day 130 / 10-29 | 实现关键词或向量检索 | G12「如何检索」 | 查询返回来源 |
| ⬜ | Day 131 / 10-30 | 实现更新、去重和冲突处理 | G12「如何更新」；T01 `s09_memory/code.py` 的提取与 consolidation | 相同事实不重复 |
...
| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ⬜ | Day 134 / 11-02 | 学 state、node、edge、conditional edge | R14「Overview」；T03 第 6 章 LangGraph 小节 | 画 Paper Agent 状态图 |
...
| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ⬜ | Day 141 / 11-09 | 理解 host/client/server 和 MCP 边界 | R15；T03 第 10 章 MCP 小节 | 画调用链 |
| ⬜ | Day 142 / 11-10 | 区分 function calling、tool、Skill、MCP | G13「与 Function Calling 的关系」 | 写对比表 |
| ⬜ | Day 143 / 11-11 | 将 `search_papers` 封装为 MCP 工具 | G13「最小实践路线」；T01 `s19_mcp_plugin/README.md` 的 MCP 接入部分 | 客户端可发现并调用 |
| ⬜ | Day 144 / 11-12 | 增加 schema、分页、超时和错误 | G13「Server 应该怎么设计」 | 返回不过载 |
| ⬜ | Day 145 / 11-13 | 增加权限与不可信输出标记 | G13「安全风险」 | 权限路径可测试 |
| ⬜ | Day 146 / 11-14 | 写 `paper-review` Skill | G29「Stage 5」；T01 `s07_skill_loading/README.md` | 含触发、步骤、模板、验收 |
...
| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ⬜ | Day 148 / 11-16 | 理解 Model、Agent 与 Harness 的不同定义 | G15「什么是 Harness」；T01 根 README 的 Harness 观点 | 标出现有模块并记录术语差异 |
...
| ⬜ | Day 153 / 11-21 | 统一 CLI/API 事件流 | G15「L6」；T02 `sessions/zh/s04_channels.md` | 两入口复用同一 core |
| ⬜ | Day 154 / 11-22 | 故障注入并周复盘 | G15「可靠性六件套」；T01 `s20_comprehensive/README.md` 选读 | 超时、坏 JSON、空结果均可处理 |
...
| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ⬜ | Day 155 / 11-23 | 理解 task/trial/grader/transcript/outcome | G17「核心抽象」；T03 第 12 章评估框架相关小节 | 画组件图 |
...
| ⬜ | Day 164 / 12-02 | 设计 read/write/network/execute 权限 | G15「Permission Tier」；T01 `s03_permission/code.py` | 默认最小权限 |
...
| ⬜ | Day 172 / 12-10 | 增加 timeout、指数退避和 jitter | T01 `s11_error_recovery/README.md`；T02 `sessions/zh/s09_resilience.md` 仅作对照 | 临时失败可恢复 |
| ⬜ | Day 173 / 12-11 | 增加幂等 key 和重复请求保护 | G15「Idempotency」；T02 `sessions/zh/s08_delivery.md` | 重试不重复写入 |
| ⬜ | Day 174 / 12-12 | 导出 Prometheus 指标 | R21 | 可查看计数和延迟 |
| ⬜ | Day 175 / 12-13 | 做故障注入报告并周复盘 | G19「Step 5」；T02 `sessions/zh/s10_concurrency.md` 选读 | 每类故障有恢复或终止 |
...
| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ⬜ | Day 176 / 12-14 | 写 Web Agent 安全边界和 spec | G21「安全边界」「MVP 范围」；T03 `Extra-Chapter/Extra11-WebAgent科普与实战.md` 选读 | 不含登录、支付、发布 |
...
| ⬜ | Day 184 / 12-22 | 设计 research→write→review 流程 | G14；T01 `s06_subagent/README.md` | 每个角色有 schema |
| ⬜ | Day 185 / 12-23 | 实现单 Agent baseline | G22 | 保存质量、成本、延迟 |
| ⬜ | Day 186 / 12-24 | 实现双角色版本 | G14；T01 `s15_agent_teams/README.md` | 上下文隔离 |
| ⬜ | Day 187 / 12-25 | 增加最多两轮 review | G14；T01 `s16_team_protocols/README.md` | 不无限循环 |
...
| 状态 | Day / 日期 | 60 分钟安排 | 学习资源 | 当日验收 |
|:---:|:---|:---|:---|:---|
| ⬜ | Day 218 / 01-25 | 浏览 SFT→RM→PPO/DPO/GRPO 完整链路 | G32「1.2 完整链路」；T03 第 11 章 Agentic RL 相关小节 | 画 Post-Training 地图 |
...
| 模拟面试 | 0 / 2 |
| 有效投递 | 0 |
| 当前最大阻塞 | 无；真实 provider 通过 Adapter + Factory 接入，密钥文件已忽略 |
| 下一步 | 执行 Day 32：比较 system/user 消息与参数变化 |
...
- 本计划的默认生命周期分支为 `original-main`；后续计划与学习记录均在该分支维护。
...
| v1.4 | 2026-06-23 | 将 Learn Claude Code、claw0、hello-agents 作为分层教程资源嵌入对应学习日；增加教程日 60 分钟模板、只读复现、官方资料优先和动态删减规则 | 用户确认采用“AgentGuide 主线 + 三仓按需融合”，并允许后续随学习情况灵活调整 |
| v1.5 | 2026-06-25 | 增加菜鸟教程 Python3 作为中文辅助资料；将 Python 前两周调整为“基础复健 + 工程高级特性补盲区”；Day 3 未完成时仪表盘改为补做而非顺延标完成 | 用户说明不是 Python 完全零基础，而是长期未用，需要温习基础并加强 OOP 及后续高级特性 |
